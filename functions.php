...
function popperscores_scripts() {
	...
	if ( is_front_page() ) {
		...
		wp_enqueue_style( 'zozotabs-css', get_template_directory_uri() . '/css/zozo.tabs.min.css' );

		wp_enqueue_style( 'zozoaccord-css', get_template_directory_uri() . '/css/zozo.accordion.min.css' );

		wp_enqueue_script( 'zozo-script', get_template_directory_uri() . '/js/zozo.tabs.min.js', array('jquery'), '', true );

		wp_enqueue_script( 'zozo-script-accord', get_template_directory_uri() . '/js/zozo.accordion.min.js', array('jquery'), '', true );

	}

	wp_enqueue_script( 'themejs', get_template_directory_uri() . '/js/theme.js', array('jquery'), '', true );	

	// Unregister standard WP jQuery & jQuery-ui
		wp_deregister_script('jquery');
		wp_deregister_script('jquery-ui-core');

	// Load jquery & jquery-ui from goolge CDN
		wp_enqueue_script('jquery', "http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://cdnjs.cloudflare.com/ajax/libs/jquery/1.11.3/jquery.min.js", false, null, true);
		wp_enqueue_script('jquery-ui', "http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.11.4/jquery-ui.min.js", false, null, true);
...
//wp_enqueue_script( 'google-maps-api', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyAEYd79ETtZFJjChhJszrQFf7oXGPyRVuI&callback=initMap', '', '', true );
	if ( is_front_page() ) {
		wp_enqueue_script( 'google-maps-api', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyAEYd79ETtZFJjChhJszrQFf7oXGPyRVuI&callback=initMap', '', '', true );
		wp_enqueue_script( 'popperscores-gmap', get_template_directory_uri() . '/js/gmap.js', array(), '', true);	
	}
